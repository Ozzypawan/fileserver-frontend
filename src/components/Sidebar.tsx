import { HardDrive, Files, Upload, FileImage, FileVideo, FileAudio, FileType2, ScrollText, FileSpreadsheet, FileText, File } from 'lucide-react';
...
const TYPE_META: { cat: string; label: string; Icon: React.ElementType; color: string; bg: string }[] = [
...
export default function Sidebar({ totalFiles, totalSize, onUploadClick, onHome, allFiles }: Props) {
    ...
    <aside className="w-60 shrink-0 bg-white border-r border-slate-200 flex flex-col h-screen sticky top-0">
      ...
      <div className="px-6 py-5 border-b border-slate-100">
        <button onClick={onHome} className="flex items-center gap-2.5 hover:opacity-80 transition-opacity">
          ...
          <span className="font-semibold text-slate-900 text-base tracking-tight">Fileserver</span>
        </button>
      </div>
      ...
      <div className="px-4 py-5 border-t border-slate-100 mt-auto">
        ...
        <div className="space-y-1.5">
          ...
          <p className="text-xs text-slate-400">of 500 MB shown</p>
        </div>
      </div>
    </aside>
  );
